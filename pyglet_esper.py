...
class MovementProcessor(esper.Processor):
    ...
    def process(self, dt):
        # This will iterate over every Entity that has BOTH of these
        # components:
        for ent, (vel, rend) in self.world.get_components(Velocity,
                                                          Renderable):
            # Update the Renderable Component's position by it's Velocity:
            # An example of keeping the sprite inside screen boundaries.
            # Basically,
            # adjust the position back inside screen boundaries if it is
            # outside:
...
world.add_component(player,
                    Renderable(sprite=pyglet.sprite.Sprite(img=player_image,
                                                           x=100,
                                                           y=100,
                                                           batch=batch)))
...
world.add_component(enemy,
                    Renderable(sprite=pyglet.sprite.Sprite(img=enemy_image,
                                                           x=400,
                                                           y=250,
                                                           batch=batch)))

# Create some Processor instances, and asign them to the World to be processed:
movement_processor = MovementProcessor(minx=0, miny=0, maxx=RESOLUTION[0],
                                       maxy=RESOLUTION[1])
...
if __name__ == "__main__":
    ...
    #        to world.process, so you must make sure that your Processor
    # classes
